...
void rp_run(void) {
    ...
    if(smokeTest) {
        ...
        rp_lock(); rp_replace_text("agent",5,NULL,0);
        copy=rp_copy_text(&length); valid=valid && copy && strcmp(copy,"agent")==0; rp_free_text(copy);
        busy=0; SendMessageW(editor,EM_SETREADONLY,FALSE,0);
        SendMessageW(editor,EM_UNDO,0,0);
        copy=rp_copy_text(&length); valid=valid && copy && length>0 && copy[length-1]=='!'; rp_free_text(copy);
        SendMessageW(editor,EM_UNDO,0,0);
        copy=rp_copy_text(&length); valid=valid && copy && strcmp(copy,sample)==0; rp_free_text(copy);
...
static LONG map_position(LONG position,const RpHunk *hunks,size_t count) {
    ptrdiff_t delta=0;
    for(size_t i=0;i<count;i++) {
        ...
        delta+=(afterEnd-afterStart)-(end-start);
    }
    return position+(LONG)delta;
}
void rp_replace_text(const char *value,size_t length,const RpHunk *hunks,size_t hunkCount) {
    if(readonlyDocument || length>INT_MAX) return;
    ...
    selection.cpMin=map_position(selection.cpMin,hunks,hunkCount);
    selection.cpMax=map_position(selection.cpMax,hunks,hunkCount);
    updating=1;
